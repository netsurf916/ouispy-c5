...
/**
 * @brief A unique matched MAC retained to prevent packet-count inflation.
 */
typedef struct
{
    uint8_t mac[6];
...
{
    { { 0xB4, 0x1E, 0x52 }, CATEGORY_FLOCK }, { { 0x70, 0xC9, 0x4E }, CATEGORY_FLOCK },
    { { 0x3C, 0x91, 0x80 }, CATEGORY_FLOCK }, { { 0xD8, 0xF3, 0xBC }, CATEGORY_FLOCK },
    { { 0x80, 0x30, 0x49 }, CATEGORY_FLOCK }, { { 0xB8, 0x35, 0x32 }, CATEGORY_FLOCK },
    { { 0x14, 0x5A, 0xFC }, CATEGORY_FLOCK }, { { 0x74, 0x4C, 0xA1 }, CATEGORY_FLOCK },
    { { 0x08, 0x3A, 0x88 }, CATEGORY_FLOCK }, { { 0x9C, 0x2F, 0x9D }, CATEGORY_FLOCK },
    { { 0xC0, 0x35, 0x32 }, CATEGORY_FLOCK }, { { 0x94, 0x08, 0x53 }, CATEGORY_FLOCK },
    { { 0xE4, 0xAA, 0xEA }, CATEGORY_FLOCK }, { { 0xF4, 0x6A, 0xDD }, CATEGORY_FLOCK },
    { { 0xF8, 0xA2, 0xD6 }, CATEGORY_FLOCK }, { { 0x24, 0xB2, 0xB9 }, CATEGORY_FLOCK },
    { { 0x00, 0xF4, 0x8D }, CATEGORY_FLOCK }, { { 0xD0, 0x39, 0x57 }, CATEGORY_FLOCK },
    { { 0xE8, 0xD0, 0xFC }, CATEGORY_FLOCK }, { { 0xE0, 0x4F, 0x43 }, CATEGORY_FLOCK },
    { { 0xB8, 0x1E, 0xA4 }, CATEGORY_FLOCK }, { { 0x70, 0x08, 0x94 }, CATEGORY_FLOCK },
    { { 0x58, 0x8E, 0x81 }, CATEGORY_FLOCK }, { { 0xEC, 0x1B, 0xBD }, CATEGORY_FLOCK },
    { { 0x3C, 0x71, 0xBF }, CATEGORY_FLOCK }, { { 0x58, 0x00, 0xE3 }, CATEGORY_FLOCK },
    { { 0x90, 0x35, 0xEA }, CATEGORY_FLOCK }, { { 0x5C, 0x93, 0xA2 }, CATEGORY_FLOCK },
    { { 0x64, 0x6E, 0x69 }, CATEGORY_FLOCK }, { { 0x48, 0x27, 0xEA }, CATEGORY_FLOCK },
    { { 0xA4, 0xCF, 0x12 }, CATEGORY_FLOCK },
    { { 0x00, 0x25, 0xDF }, CATEGORY_BODY_CAM },
    { { 0x0C, 0x9A, 0xE6 }, CATEGORY_DRONE }, { { 0x8C, 0x58, 0x23 }, CATEGORY_DRONE },
    { { 0x04, 0xA8, 0x5A }, CATEGORY_DRONE }, { { 0x58, 0xB8, 0x58 }, CATEGORY_DRONE },
    { { 0xE4, 0x7A, 0x2C }, CATEGORY_DRONE }, { { 0x60, 0x60, 0x1F }, CATEGORY_DRONE },
    { { 0x48, 0x1C, 0xB9 }, CATEGORY_DRONE }, { { 0x34, 0xD2, 0x62 }, CATEGORY_DRONE },
    { { 0x00, 0x12, 0x1C }, CATEGORY_DRONE }, { { 0x00, 0x26, 0x7E }, CATEGORY_DRONE },
    { { 0x90, 0x03, 0xB7 }, CATEGORY_DRONE }, { { 0x90, 0x3A, 0xE6 }, CATEGORY_DRONE },
    { { 0xA0, 0x14, 0x3D }, CATEGORY_DRONE }, { { 0x38, 0x1D, 0x14 }, CATEGORY_DRONE },
    { { 0x18, 0x7F, 0x88 }, CATEGORY_DOORBELL_CAMERA }, { { 0x24, 0x2B, 0xD6 }, CATEGORY_DOORBELL_CAMERA },
    { { 0x34, 0x3E, 0xA4 }, CATEGORY_DOORBELL_CAMERA }, { { 0x54, 0xE0, 0x19 }, CATEGORY_DOORBELL_CAMERA },
    { { 0x5C, 0x47, 0x5E }, CATEGORY_DOORBELL_CAMERA }, { { 0x64, 0x9A, 0x63 }, CATEGORY_DOORBELL_CAMERA },
    { { 0x90, 0x48, 0x6C }, CATEGORY_DOORBELL_CAMERA }, { { 0x9C, 0x76, 0x13 }, CATEGORY_DOORBELL_CAMERA },
    { { 0xAC, 0x9F, 0xC3 }, CATEGORY_DOORBELL_CAMERA }, { { 0xC4, 0xDB, 0xAD }, CATEGORY_DOORBELL_CAMERA },
    { { 0xCC, 0x3B, 0xFB }, CATEGORY_DOORBELL_CAMERA },
    { { 0x7C, 0x2A, 0x9E }, CATEGORY_SMARTGLASSES }, { { 0xCC, 0x66, 0x0A }, CATEGORY_SMARTGLASSES },
    { { 0xF4, 0x03, 0x43 }, CATEGORY_SMARTGLASSES }, { { 0x5C, 0xE9, 0x1E }, CATEGORY_SMARTGLASSES },
...
void app_main( void )
{
    ...
    while( 1 )
    {
        wifi_monitor_band( WIFI_BAND_MODE_2G_ONLY, wifi_2g_channels, WIFI_2G_CHANNEL_COUNT );
        wifi_monitor_band( WIFI_BAND_MODE_5G_ONLY, wifi_5g_channels, WIFI_5G_CHANNEL_COUNT );
    }
}
...
static void wifi_promiscuous_rx( void *a_buffer, wifi_promiscuous_pkt_type_t a_type )
{
    if( a_buffer == NULL || ( a_type != WIFI_PKT_MGMT && a_type != WIFI_PKT_DATA ) )
    {
        return;
    }

    const wifi_promiscuous_pkt_t *packet = (const wifi_promiscuous_pkt_t *)a_buffer;
    ...
    for( size_t i = 0; i < OUI_DATABASE_COUNT; i++ )
    {
        if( memcmp( a_mac, oui_database[i].prefix, 3 ) == 0 )
        {
            ...
            if( oui_remember_mac( a_mac, category ) )
            {
                ...
                ESP_LOGI( TAG, "New %s device (%u total)",
                          categories[category].name, categories[category].count );
            }
            return;
        }
    }
...
/**
 * @brief Remember a matched MAC address if it has not already been counted.
 * @param a_mac Six-byte MAC address.
 * @param a_category Category associated with the MAC.
 * @return true when the MAC was newly stored; false when already known/full.
 */
static bool oui_remember_mac( const uint8_t *a_mac, oui_category_t a_category )
{
    ...
    for( size_t i = 0; i < MAX_OBSERVED_MACS; i++ )
    {
        ...
        {
            if( memcmp( observed_macs[i].mac, a_mac, sizeof( observed_macs[i].mac ) ) == 0 )
            {
                return false;
            }
        }
    ...
    memcpy( observed_macs[free_slot].mac, a_mac, sizeof( observed_macs[free_slot].mac ) );
...
/**
 * @brief Select the status color for a category count.
 */
static uint16_t category_color( uint16_t a_count )
{
    if( a_count == 0 )
...
/**
 * @brief Draw category counts and relative prevalence bars.
 */
static esp_err_t lcd_draw_status( void )
{
    ...
    lcd_set_cursor( 4, STATUS_TITLE_Y );
    ...
    for( size_t i = 0; i < CATEGORY_COUNT; i++ )
    {
        ...
        snprintf( count_text, sizeof( count_text ), "%u", categories[i].count );
        lcd_set_cursor( STATUS_COUNT_X, y );
        ESP_ERROR_CHECK( lcd_draw_text( count_text, color, LCD_BLACK ) );
...
/**
 * @brief Hop all supplied channels while periodically refreshing the LCD.
 */
static void wifi_monitor_band( wifi_band_mode_t a_band,
                               const uint8_t *a_channels,
                               size_t a_channel_count )
